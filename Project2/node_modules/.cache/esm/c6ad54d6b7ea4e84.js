let isValidCommand,parseStringWithIndefiniteNumberOfParams,getValueForClosestKey,getClosestElemWithKey,getTargetElemsForKeyName,getValueForKeyName,forEachAttr,showWarning,optionsData;_b7e‍.x([["callWatchFunctionsOnElements",()=>callWatchFunctionsOnElements],["triggerWatchAttributes",()=>triggerWatchAttributes],["executeCommandOnMultipleElements",()=>executeCommandOnMultipleElements],["getTargetsForCommand",()=>getTargetsForCommand]]);_b7e‍.w("../common/get-valid-element-properties",[["isValidCommand",["isValidCommand"],function(v){isValidCommand=v}]]);_b7e‍.w("../parse-data-attributes",[["parseStringWithIndefiniteNumberOfParams",["parseStringWithIndefiniteNumberOfParams"],function(v){parseStringWithIndefiniteNumberOfParams=v}]]);_b7e‍.w("./getAndSetKeyValues",[["getValueForClosestKey",["getValueForClosestKey"],function(v){getValueForClosestKey=v}],["getClosestElemWithKey",["getClosestElemWithKey"],function(v){getClosestElemWithKey=v}],["getTargetElemsForKeyName",["getTargetElemsForKeyName"],function(v){getTargetElemsForKeyName=v}],["getValueForKeyName",["getValueForKeyName"],function(v){getValueForKeyName=v}]]);_b7e‍.w("../hummingbird/lib/dom",[["forEachAttr",["forEachAttr"],function(v){forEachAttr=v}]]);_b7e‍.w("../common/show-error",[["showWarning",["showWarning"],function(v){showWarning=v}]]);_b7e‍.w("../inputjs/optionsData",[["default",["optionsData"],function(v){optionsData=v}]]);
...
// e.g. callWatchFunctionsOnElements(document.querySelectorAll("[watch]"))
  ...
  elems.forEach(elem => {
    _b7e‍.a("forEachAttr",forEachAttr)(elem, (attrName, attrValue) => {
      if (attrName.indexOf("watch:") === 0) {
        let dashCaseKeyName = attrName.substring("watch:".length);
        let closestElemWithKey = _b7e‍.a("getClosestElemWithKey",getClosestElemWithKey)({ elem, keyName: dashCaseKeyName });
        let value = _b7e‍.a("getValueForKeyName",getValueForKeyName)({ elem: closestElemWithKey, keyName: dashCaseKeyName });
        let targetElems = _b7e‍.a("getTargetElemsForKeyName",getTargetElemsForKeyName)({ elem, keyName: dashCaseKeyName });
...
// note: this allows for watching multiple keys with the same name as long as they're at different levels
  ...
  watchElems.forEach(watchElem => {
    ...
    let listOfCommands = _b7e‍.a("parseStringWithIndefiniteNumberOfParams",parseStringWithIndefiniteNumberOfParams)(watchAttrValue);
    listOfCommands.forEach(({ funcName, args }) => {
      if (_b7e‍.a("isValidCommand",isValidCommand)({ commandName: funcName })) {
        executeCommand({ elem: watchElem, commandName: funcName, value, method: "set" });
      } else {
        // call custom watch function
        let watchFunc = _b7e‍.a("optionsData",optionsData).watchFunctions && _b7e‍.a("optionsData",optionsData).watchFunctions[funcName];
...
//     native prop: get/set a property on the current element
  ...
  for (let i = 0; i < targetElems.length; i++) {
    ...
    if (_b7e‍.a("isValidCommand",isValidCommand)({ commandName })) {
      if (method === "set") {
...
function executeCommand({ elem, commandName, value, method }) {
  if (commandName.indexOf("@attr:") === 0) {
    ...
    if (prop.toLowerCase() === "src" && elem.tagName.toLowerCase() === "img") {
      _b7e‍.a("showWarning",showWarning)(
