_834‍.x([["getReservedWordInfo",()=>getReservedWordInfo]]);const path = require("upath");
const jsonfile = require("jsonfile");

let usernameCantBeTheseWords = jsonfile.readFileSync(
  path.join(__dirname, "../config/username-cant-be-these-words.json")
);
let usernameCantBeThesePluralizedWords = usernameCantBeTheseWords.map(w => w + "s");
let usernameCantIncludeTheseWords = jsonfile.readFileSync(
  path.join(__dirname, "../config/usernames-cant-include-these-words.json")
);
...
let usernameCantIncludeTheseWordsLength = usernameCantIncludeTheseWords.length;
       ...
       function getReservedWordInfo(word) {
