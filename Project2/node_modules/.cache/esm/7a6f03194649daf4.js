let isValidCommand,showError,callSaveFunction,triggerWatchAttributes,executeCommandOnMultipleElements,getTargetsForCommand,forEachAttr;_464‍.x([["getTargetElemsForKeyName",()=>getTargetElemsForKeyName],["setValueForKeyName",()=>setValueForKeyName],["getValueForKeyName",()=>getValueForKeyName],["getClosestElemWithKey",()=>getClosestElemWithKey],["getValueForClosestKey",()=>getValueForClosestKey],["setValueForClosestKey",()=>setValueForClosestKey],["getKeyNamesFromElem",()=>getKeyNamesFromElem]]);_464‍.w("../common/get-valid-element-properties",[["isValidCommand",["isValidCommand"],function(v){isValidCommand=v}]]);_464‍.w("../common/show-error",[["showError",["showError"],function(v){showError=v}]]);_464‍.w("../inputjs/onSave",[["callSaveFunction",["callSaveFunction"],function(v){callSaveFunction=v}]]);_464‍.w("./commandAndWatchHelpers",[["triggerWatchAttributes",["triggerWatchAttributes"],function(v){triggerWatchAttributes=v}],["executeCommandOnMultipleElements",["executeCommandOnMultipleElements"],function(v){executeCommandOnMultipleElements=v}],["getTargetsForCommand",["getTargetsForCommand"],function(v){getTargetsForCommand=v}]]);_464‍.w("../hummingbird/lib/dom",[["forEachAttr",["forEachAttr"],function(v){forEachAttr=v}]]);
...
function _valueForKeyName({ method, elem, keyName, value = "" }) {
  ...
  if (!normalKeyExists && !temporaryKeyExists) {
    _464‍.a("showError",showError)("You can't set a key name to a value if the key doesn't exist on the element yet");
    return;
  }

  let attrName = normalKeyExists ? "key:" + dashCaseKeyName : "temporary:key:" + dashCaseKeyName;
  let attrValue = elem.getAttribute(attrName);
  let hasValidCommand = _464‍.a("isValidCommand",isValidCommand)({ commandName: attrValue, includingSearchCommand: true });
  ...
  if (!hasValidCommand) {
    ...
    let targets = _464‍.a("getTargetsForCommand",getTargetsForCommand)({ elem, dashCaseKeyName, attrName, attrValue });
    targetElems = targets.targetElems;
    targetAttr = targets.targetAttr;
    if (method === "set") {
      _464‍.a("executeCommandOnMultipleElements",executeCommandOnMultipleElements)({ targetElems, targetAttr, value, method });
    } else {
      return _464‍.a("executeCommandOnMultipleElements",executeCommandOnMultipleElements)({ targetElems, targetAttr, value, method });
    }
  }
  ...
  if (method === "set") {
    ...
    if (!settingTemporaryKey) {
      _464‍.a("callSaveFunction",callSaveFunction)(elem);
    }
    ...
    _464‍.a("triggerWatchAttributes",triggerWatchAttributes)({ elem, dashCaseKeyName, value, targetElems });
  }
}
...
// returns keynames with dashes in them: ["example-key", "another-example"]
  ...
  _464‍.a("forEachAttr",forEachAttr)(elem, attrName => {
