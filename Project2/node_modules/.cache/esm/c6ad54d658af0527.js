let isValidCommand,parseStringWithIndefiniteNumberOfParams,getValueForClosestKey,getClosestElemWithKey,getTargetElemsForKeyName,getValueForKeyName,forEachAttr,showWarning,optionsData;_58a‍.x([["callWatchFunctionsOnElements",()=>callWatchFunctionsOnElements],["triggerWatchAttributes",()=>triggerWatchAttributes],["executeCommandOnMultipleElements",()=>executeCommandOnMultipleElements],["getTargetsForCommand",()=>getTargetsForCommand]]);_58a‍.w("../common/get-valid-element-properties",[["isValidCommand",["isValidCommand"],function(v){isValidCommand=v}]]);_58a‍.w("../parse-data-attributes",[["parseStringWithIndefiniteNumberOfParams",["parseStringWithIndefiniteNumberOfParams"],function(v){parseStringWithIndefiniteNumberOfParams=v}]]);_58a‍.w("./getAndSetKeyValues",[["getValueForClosestKey",["getValueForClosestKey"],function(v){getValueForClosestKey=v}],["getClosestElemWithKey",["getClosestElemWithKey"],function(v){getClosestElemWithKey=v}],["getTargetElemsForKeyName",["getTargetElemsForKeyName"],function(v){getTargetElemsForKeyName=v}],["getValueForKeyName",["getValueForKeyName"],function(v){getValueForKeyName=v}]]);_58a‍.w("../hummingbird/lib/dom",[["forEachAttr",["forEachAttr"],function(v){forEachAttr=v}]]);_58a‍.w("../common/show-error",[["showWarning",["showWarning"],function(v){showWarning=v}]]);_58a‍.w("../inputjs/optionsData",[["default",["optionsData"],function(v){optionsData=v}]]);
...
// e.g. callWatchFunctionsOnElements(document.querySelectorAll("[watch]"))
  ...
  elems.forEach(elem => {
    _58a‍.a("forEachAttr",forEachAttr)(elem, (attrName, attrValue) => {
      if (attrName.indexOf("watch:") === 0) {
        let dashCaseKeyName = attrName.substring("watch:".length);
        let closestElemWithKey = _58a‍.a("getClosestElemWithKey",getClosestElemWithKey)({ elem, keyName: dashCaseKeyName });
        let value = _58a‍.a("getValueForKeyName",getValueForKeyName)({ elem: closestElemWithKey, keyName: dashCaseKeyName });
        let targetElems = _58a‍.a("getTargetElemsForKeyName",getTargetElemsForKeyName)({ elem, keyName: dashCaseKeyName });
...
// note: this allows for watching multiple keys with the same name as long as they're at different levels
  ...
  watchElems.forEach(watchElem => {
    ...
    let listOfCommands = _58a‍.a("parseStringWithIndefiniteNumberOfParams",parseStringWithIndefiniteNumberOfParams)(watchAttrValue);
    listOfCommands.forEach(({ funcName, args }) => {
      if (_58a‍.a("isValidCommand",isValidCommand)({ commandName: funcName })) {
        executeCommand({ elem: watchElem, commandName: funcName, value, method: "set" });
      } else {
        // call custom watch function
        let watchFunc = _58a‍.a("optionsData",optionsData).watchFunctions && _58a‍.a("optionsData",optionsData).watchFunctions[funcName];
...
//     native prop: get/set a property on the current element
  ...
  for (let i = 0; i < targetElems.length; i++) {
    ...
    if (_58a‍.a("isValidCommand",isValidCommand)({ commandName })) {
      if (method === "set") {
...
function executeCommand({ elem, commandName, value, method }) {
  if (commandName.indexOf("@attr:") === 0) {
    ...
    if (prop.toLowerCase() === "src" && elem.tagName.toLowerCase() === "img") {
      _58a‍.a("showWarning",showWarning)(
